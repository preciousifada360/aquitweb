...
$app->group('/coach',function($app){
    ...
    $app->map(['get','post'],'/new',function($req,$res){
        if($req->isGet()){
            ...
            $body->write($this->view->render('aquits/app.joinaquits.com/coach/new.html', $store->getTemplateData()));
            return $res;
        }
        ...
        if($req->ispost()){
            ...
            if($this->get('controller')->isOk($response)){
                ...
                return $res->withRedirect('/dashboard/coach/new');
            } 
        }
    })->setName('coach');
